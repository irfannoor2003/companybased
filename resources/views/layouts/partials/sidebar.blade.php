...
<aside
    ...
    :class="collapsed && !hover ? 'lg:w-16' : 'lg:w-64'"
>
    {{-- Brand --}}
    <div class="flex h-16 shrink-0 items-center gap-3 border-b border-line px-4">
        <a href="{{ route('dashboard') }}" class="flex min-w-0 items-center gap-3">
            ...
            @endif
            <span class="truncate text-base font-bold tracking-tight text-ink" x-show="hover || !collapsed">{{ $appBrand['companyName'] }}</span>
        </a>
    ...
    {{-- Navigation --}}
    <nav class="flex-1 overflow-y-auto px-3 py-4" :class="collapsed && !hover ? 'lg:px-2' : ''">
        <ul class="space-y-1">
            @foreach ($navItems as $nav)
                @if ($nav['visible'])
                            ...
                            >
                                <span class="truncate">{{ $nav['item']['group'] }}</span>
                                <x-icon name="chevron-down" class="ml-auto size-4 transition-transform" ::class="groupOpen && 'rotate-180'" />
                            </button>
                            <ul x-show="groupOpen" x-collapse class="mt-1 space-y-0.5">
                                @foreach ($nav['children'] as $child)
                                        <li>
                                            <a
                                                href="{{ route($child['route']) }}"
                                                @click="drawerOpen = false"
                                                :class="collapsed && !hover ? 'lg:justify-center lg:px-0' : ''"
                                            ...
                                            >
                                                ...
                                                <span class="truncate" x-show="hover || !collapsed">{{ $child['label'] }}</span>
                                            </a>
                                        </li>
                                @endforeach
                            </ul>
                    ...
                    @else
                        ...
                        <li>
                            ...
                            >
                                ...
                                <span class="truncate" x-show="hover || !collapsed">{{ $nav['item']['label'] }}</span>
                            </a>
                        </li>
                    @endif
                @endif
    ...
    <div class="border-t border-line p-3">
        <button
            type="button"
            @click="collapsed = !collapsed; hover = false"
        ...
        >
            ...
            <span x-show="hover || !collapsed">Collapse</span>
        </button>

        <a href="{{ route('profile.edit') }}" class="mt-1 flex items-center gap-3 rounded-lg px-3 py-2 transition-colors hover:bg-surface-muted" :class="collapsed && !hover ? 'lg:justify-center lg:px-0' : ''">
            <div class="flex size-8 shrink-0 items-center justify-center rounded-full bg-primary/15 text-xs font-bold text-primary">
                {{ auth()->user()->initials() }}
            </div>
            <div class="min-w-0" x-show="hover || !collapsed">
