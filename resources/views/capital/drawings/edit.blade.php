        ...
        <x-card>
            <form method="POST" action="{{ route('capital.drawings.update', $drawing) }}" class="space-y-5">
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    ...
                    <x-input name="recipient" label="Recipient" required placeholder="e.g. Kwame Mensah" value="{{ old('recipient', $drawing->recipient) }}" :error="$errors->first('recipient')" />
                </div>
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    ...
                    <x-input name="currency" label="Currency" placeholder="GHS, USD, EUR" value="{{ old('currency', $drawing->currency) }}" :error="$errors->first('currency')" />
                </div>
