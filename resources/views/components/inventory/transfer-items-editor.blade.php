...
>
    <div class="mb-3 flex items-center justify-between gap-3">
        <p class="text-sm font-semibold text-ink">Items to transfer</p>
        @if (! $locked)
    ...
    <template x-if="items.length === 0">
        <p class="rounded-lg border border-dashed border-line px-4 py-6 text-center text-sm text-ink-faint">
            No items yet. Click “Add item” to list what is moving between warehouses.
        </p>
    </template>
    ...
    <div class="space-y-2">
        <template x-for="(item, i) in items" :key="i">
            <div class="grid grid-cols-1 gap-2 rounded-lg border border-line bg-surface-muted/40 p-3 sm:grid-cols-[2fr_140px_1fr_40px]">
                ...
                <input type="text" :name="'items[' + i + '][notes]'" x-model="item.notes" :disabled="{{ $locked ? 'true' : 'false' }}" class="input" placeholder="Notes…">
                @if (! $locked)
