        ...
        <div class="invoice-document">
            <div class="flex justify-between border-b border-line pb-4 mb-6">
                ...
                <div class="text-right">
                    <h2 class="text-2xl font-bold text-ink">Purchase Invoice</h2>
            ...
            <div class="mb-6">
                <h3 class="text-sm font-semibold text-ink-faint uppercase mb-2">Bill from</h3>
                <p class="font-medium text-ink">{{ $invoice->supplier?->company_name }}</p>
                @if ($invoice->supplier?->contact_name)
                    <p class="text-sm text-ink-soft">{{ $invoice->supplier->contact_name }}</p>
                @endif
                @if ($invoice->supplier?->address)
                    <p class="text-sm text-ink-soft">{{ $invoice->supplier->address }}</p>
                @endif
                @if ($invoice->supplier?->tax_number)
                    <p class="text-sm text-ink-soft">Tax: {{ $invoice->supplier->tax_number }}</p>
                @endif
                @if ($invoice->supplier?->short_code)
                    <p class="text-sm text-ink-soft">Code: {{ $invoice->supplier->short_code }}</p>
                @endif
            </div>
            ...
            @if ($invoice->payments->isNotEmpty())
                <div class="mb-6">
                    <h3 class="text-sm font-semibold text-ink-faint uppercase mb-3">Payments made</h3>
