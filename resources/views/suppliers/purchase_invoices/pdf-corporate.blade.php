...
* { box-sizing: border-box; }
        ...
        .meta-row .k { display: inline-block; width: 104px; color: #64748b; font-size: 9.5px; text-transform: uppercase; letter-spacing: 0.08em; }
    ...
    <div class="page">
        ...
        <div class="head">
            <table width="100%" cellpadding="0" cellspacing="0" border="0"><tr>
                ...
                <td width="240" class="doc-label">
                    <div class="kind">Purchase invoice</div>
                    <h1>{{ $invoice->number }}</h1>
                    <div class="number">VENDOR BILL</div>
        ...
        <div class="body">
            <table class="parties" cellpadding="0" cellspacing="0" border="0">
                <tr>
                    <td style="width:48%">
                        <div class="block">
                            <div class="block-label">Bill from</div>
                            <p class="name">{{ $invoice->supplier?->company_name ?: '—' }}</p>
                            @if ($invoice->supplier?->contact_name)
                                <p>{{ $invoice->supplier->contact_name }}</p>
                            @endif
                            ...
                            @if ($invoice->supplier?->short_code)
                                <p>Code: {{ $invoice->supplier->short_code }}</p>
                            @endif
                        </div>
                    </td>
                    <td class="divider"></td>
                    <td style="width:48%">
                        ...
                        @if ($invoice->order)
                            <div class="meta-row"><span class="k">Purchase order</span><span style="color:#0f172a;font-weight:600;">{{ $invoice->order->number }}</span></div>
                        @endif
                        @if ($invoice->productionOrder)
                            <div class="meta-row"><span class="k">Production order</span><span style="color:#0f172a;font-weight:600;">{{ $invoice->productionOrder->number }}</span></div>
                        @endif
                    </td>
                </tr>
            </table>
