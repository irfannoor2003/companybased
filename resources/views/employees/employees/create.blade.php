            ...
            <form method="POST" action="{{ route('employees.employees.store') }}" class="space-y-6">
                ...
                <div>
                    ...
                    <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <x-input name="email" label="Email" type="email" placeholder="employee@example.com" value="{{ old('email') }}" :error="$errors->first('email')" />
                        <p class="-mt-2 text-xs text-ink-faint sm:col-span-2">If this email matches an existing login it is linked automatically. Otherwise a login account (Employee role) will be created for it.</p>
